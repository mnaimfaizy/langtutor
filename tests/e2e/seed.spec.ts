...
/**
 * Phase 1.8 acceptance test — offline first-run seed.
 *
...
test("fresh install: SeedBootstrap shows passages and cards without the Mac", async ({ page }) => {
  await page.goto("/");
...
test("seed bar reports non-zero passage and card counts", async ({ page }) => {
  await page.goto("/");
  const seedReady = page.getByTestId("seed-ready");
  await expect(seedReady).toBeVisible({ timeout: 15_000 });

  const text = (await seedReady.textContent()) ?? "";
  // E.g. "8 passages · 20 cards ready offline"
